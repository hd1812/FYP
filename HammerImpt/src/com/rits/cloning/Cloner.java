...
public class Cloner
{
	 ...
	 * a file instead of the default behaviour which is to println(cloned) )
	 *
	 * @param dumpCloned
	 *            an implementation of the interface which can dump the cloned
	 *            classes.
	 */
		...
		for (final Field field : fields)
		{
			...
			if (!Modifier.isStatic(modifiers))
			{
				...
				} else
				{
					...
					final Object fieldObjectClone = clones != null
							? (shouldClone ? cloneInternal(fieldObject, clones) : fieldObject)
							: fieldObject;
	 ...
	 * setDumpCloned() if you want to control where to print the cloned classes.
	 *
	 ...
	 *         tomcat/shared), then this method is preferable in order to
	 *         instantiate cloner. Please see
	 *         https://code.google.com/p/cloning/issues/detail?id=23
	 */
